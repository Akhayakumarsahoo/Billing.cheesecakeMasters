...
export function AdminSettlementHistoryClient({
  ...
  const isWithin30Days = (settlementDateStr: string) => {
    const targetDate = new Date(`${settlementDateStr}T00:00:00`);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[var(--text-secondary)] mt-1">
            End of day drawer reconciliations for {outletName}
          </p>
        </div>
            ...
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent className="max-w-xs">
                  <p>
                    {role === "admin"
                      ? "Admins can edit or cancel daily settlements for dates within the last 30 days."
                      : "Managers can only edit or cancel daily settlements within 24 hours of creation."}
                  </p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
        ...
        {settlements.length === 0 ? (
          <div className="p-12 text-center space-y-4">
            ...
            <p className="text-sm text-[var(--text-secondary)] max-w-sm mx-auto">
              No drawer reconciliation records matched the selected date filter range.
            </p>
          </div>
        ) : (
                ...
                <TableBody>
                  {settlements.map((s, idx) => {
                    ...
                    const isEditable = active && (
                      role === "admin"
                        ? isWithin30Days(s.settlementDate)
                        : isWithin24Hours(s.createdAt)
                    );
                        ...
                        <TableCell className="text-right">
                          <div className="flex justify-end items-center gap-1.5">
                            {isEditable ? (
                              <>
                                <Link
                                  href={`/outlets/${outletId}/settlements/edit/${s.id}`}
                              ...
                              <TooltipProvider>
                                <Tooltip>
                                  ...
                                  <TooltipContent>
                                    <p>
                                      {role === "admin"
                                        ? "This settlement date is older than 30 days and cannot be modified."
                                        : "This settlement was created over 24 hours ago and is locked for managers."}
                                    </p>
                                  </TooltipContent>
                                </Tooltip>
                              </TooltipProvider>
                      ...
                      <div className="pt-3 border-t border-[var(--border-subtle)] space-y-3 animate-in slide-in-from-top duration-200">
                        ...
                        {!isEditable && active && (
                          <div className="bg-[var(--bg-hover)] border border-[var(--border-default)] rounded-lg p-2 text-center text-xs text-[var(--text-secondary)]">
                            This settlement is locked from modifications.
                          </div>
                        )}
                      </div>
                    )}
